...
On the left panel in the \menu{Simulation} tab, we can select which model components to plot, for instance, all reactive species.
...
%\end{minipage}
\end{wrapfigure}
Choose the tab \menu{Model} and we see the \SBML document in a tree structure that we can explore (\cref{fig:modelTree}).
...
\section{Save model}
In order to save the model with the parameter, species and compartment values that we have modified, we first have to select the tab \menu{Model}.
Then we can save the simulation data under \menu{File > Save as}.
...
For example, we can simply drag\&drop data into the application or select \menu{File > Open > Load experimental data} to open a file.
Just like in the case of model files, we can also use the key stroke combination \keys{\cmd + O} or \keys{\ctrlwin + O} depending on our \OS.
...
The \cref{fig:inputdialog} shows an example of the file input dialog. Here we open the simulation data saved in \cref{ch:savesim}.
...
In order to upload a file, click on \keys{Open}.
We can also save our configuration by clicking on \keys{Save}.

\TODO{In a configuration file, we can set the minimum and maximum values for initialization of the estimated quantities at the beginning of the estimation (initial minimum/maximum) as well as the minimum and maximum value for each estimated quantity that is allowed during the whole estimation (minimum/maximum).} Here is an example how such a file might look like:
...
Note that each tab has these buttons separately, so our choice in one tab does not affect others.
}
...
A separate dialog window will appear that offers a large number of optimization procedures \TODO{(\cref{fig:algorithmChoice})}.
...
This is the distance between our experimental data and the simulation result for the currently best parameter set.
