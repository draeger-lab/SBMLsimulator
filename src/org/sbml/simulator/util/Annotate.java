...
public class Annotate {
	...
	KeggInfoManagement manager = new KeggInfoManagement();
	KeggFunctionManagement fmanager  = new KeggFunctionManagement();
	...
	for (Species spec : doc.getModel().getListOfSpecies()) {
	    ...
	    if (pos>0) symbol = symbol.substring(0, pos);
	    
	    KeggQuery query = new KeggQuery(KeggQuery.genericFind, symbol);
	    CustomObject<Object> res = fmanager.getInformation(query);
	    boolean foundExactHit=false;
	    
	    if (res!=null && res.getObject()!=null && res.getObject().toString().length()>0) {
	...
	System.out.println("Total: " + total + " Matched: " + matched + " Ambigous: " + ambigous + " Unmatched: " + notMatched);
    ...
    private static void annotate(Species spec, String ko_id, KeggInfoManagement manager) {
