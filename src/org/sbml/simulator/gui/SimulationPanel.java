...
public class SimulationPanel extends JPanel implements
	...
	public void notifyGenerationPerformed(String[] header,
		...
		if (Double.isNaN(currentDistance)
			...
			tools.setCurrentQualityMeasure(statDoubles[runBestIndex]
					.doubleValue());
	...
	public void savePreferences() throws IllegalArgumentException,
		...
		prefs.put(SimulationOptions.SIM_START_TIME,
				foot.getSimulationStartTime());
	...
	public void simulate() throws Exception {
		SimulationToolPanel foot = getOrCreateFootPanel();
		simulate(worker.getModel(), foot.getSimulationStartTime(),
				foot.getSimulationEndTime(), foot.getStepSize());
	 ...
	 * 
	 * @see
	 * org.sbml.simulator.math.odes.DESSolver#addPropertyChangedListener(java
	 ...
	 * 
	 * @see
	 * org.sbml.simulator.math.odes.DESSolver#removePropertyChangedListener(
	...
	public void firePropertyChanged(String name, Object oldValue, Object newValue) {
		if (!this.listeners.isEmpty()) {
			PropertyChangeEvent evt = new PropertyChangeEvent(listeners, name, oldValue, newValue);
			for (PropertyChangeListener listener : this.listeners) {
				listener.propertyChange(evt);
			}
		}
