...
public class SimulationToolPanel extends JPanel implements ItemListener,
	...
	public SimulationToolPanel(SimulationWorker worker)
		...
		JButton cameraButton = new JButton(
			org.sbml.simulator.gui.GUITools.getIconCamera());
		...
		if (worker.isSetModel() && worker.isSetData()) {
			...
			qualityMeasureField
					.setToolTipText(StringUtil.toHTML(
						String.format(QUALITY_FIELD_TOOL_TIP,
							qualityMeasureFunctions.getSelectedItem(),
	...
	public Properties getProperties() {
		...
		p.put(SimulationOptions.SIM_STEP_SIZE,
			Double.valueOf(worker.getDESSolver().getStepSize()));
		p.put(SimulationOptions.SIM_MAX_STEPS_PER_UNIT_TIME,
			Integer.valueOf(maxStepsPerUnit));
		 ...
		 * Plot
		 */
		p.put(PlotOptions.SHOW_PLOT_GRID, Boolean.valueOf(showGrid.isSelected()));
		p.put(PlotOptions.PLOT_LOG_SCALE, Boolean.valueOf(logScale.isSelected()));
		p.put(PlotOptions.SHOW_PLOT_LEGEND,
			Boolean.valueOf(showLegend.isSelected()));
	...
	public void loadPreferences() throws IllegalArgumentException,
		...
		showLegend.setSelected(prefsPlot.getBoolean(PlotOptions.SHOW_PLOT_LEGEND));
