...
public class SimulationVisualizationPanel extends JSplitPane implements
  ...
  public void itemStateChanged(ItemEvent e) {
    if (e.getSource() instanceof AbstractButton) {
      ...
      if (button.getActionCommand() != null) {
        ...
        } else if (com.equals(PlotOptions.PLOT_LOG_SCALE.getOptionName())) {
          setPlotToLogScale(button);
        } else if (com.equals(PlotOptions.SHOW_PLOT_LEGEND.getOptionName())) {
          plot.setShowLegend(button.isSelected());
        } else if (com.equals(PlotOptions.SHOW_PLOT_TOOLTIPS.getOptionName())) {
   ...
   * @param connected
   * @param clearFirst
   *        if true, everything already plotted will be removed before plotting.
   */
  private void plot(MultiBlockTable data, boolean connected, boolean clearFirst) {
    String name;
  ...
  public void setExperimentData(MultiBlockTable data) {
    ...
    if (experimentData != null) {
      ...
      for (int i = 0; i < legend.getRowCount(); i++) {
        legend
            .setSelected(i, experimentData.getColumn(legend.getId(i)) != null);
      }
    }
  }
   ...
   * 
   * @param identifiers
   *        The identifiers of the variables to be selected and to occur in the
   *        plot.
   */
