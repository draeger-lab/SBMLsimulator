...
public class QuantitySelectionPanel extends JPanel implements ActionListener {
	...
	public QuantityBlock(Quantity q, JCheckBox check, JSpinner initMin,
	    ...
	    checkbox.setToolTipText(StringUtil.toHTML(String.format(
		checkBoxToolTip, className, name), 40));
	    minInitSpinner.setToolTipText(StringUtil.toHTML(String.format(
		INIT_MIN_MAX_SPINNER_TOOL_TIP, MINIMUM, className, name), 40));
	    maxInitSpinner.setToolTipText(StringUtil.toHTML(String.format(
		INIT_MIN_MAX_SPINNER_TOOL_TIP, MAXIMUM, className, name), 40));
	    minSpinner.setToolTipText(StringUtil.toHTML(String.format(
		MIN_MAX_SPINNER_TOOL_TIP, MINIMUM, className, name), 40));
	    maxSpinner.setToolTipText(StringUtil.toHTML(String.format(
	...
	tabs.setToolTipTextAt(3, StringUtil.toHTML(String.format(TAB_TOOL_TIP,
	    LOCAL_PARAMETERS), 40));
	tabs.setEnabledAt(2, model.getNumParameters() > 0);
	tabs.setToolTipTextAt(2, StringUtil.toHTML(String.format(TAB_TOOL_TIP,
	    GLOBAL_PARAMETERS), 40));
	tabs.setEnabledAt(1, model.getNumSpecies() > 0);
	tabs.setToolTipTextAt(1, StringUtil.toHTML(String.format(TAB_TOOL_TIP,
	    SPECIES), 40));
	tabs.setEnabledAt(0, model.getNumCompartments() > 0);
	tabs.setToolTipTextAt(0, StringUtil.toHTML(String.format(TAB_TOOL_TIP,
	    COMPARTMENTS), 40));
	int i = 3;
	...
	lh.add(true, new JLabel(), new JLabel(
	    "<html>initial<br/>minimum</html>"), new JLabel(
	    "<html>initial<br/>maximum</html>"), sep, new JLabel(
	...
	for (Quantity q : listOfQuantities) {
		...
		maxInit, min, max);
	    lh.add(true, chck, minInit, maxInit, sep, min, max, sep,
		new JLabel(StringTools
			.concat("<html>",
