...
public class ASTNodeInterpreter implements ASTNodeCompiler {
    ...
    public ASTNodeValue compile(CallableSBase nsb) throws SBMLException {
		if (nsb instanceof Species) {
			...
			if (valueHolder.getCurrentCompartmentSize(nsb.getId()) == 0d) {
				return new ASTNodeValue(valueHolder.getCurrentSpeciesValue(nsb.getId()), this);
			}

			if (s.isSetInitialAmount() && !s.getHasOnlySubstanceUnits()) {
				return new ASTNodeValue(valueHolder.getCurrentSpeciesValue(nsb.getId())
						/ valueHolder.getCompartmentValueOf(nsb.getId()), this);
			}
			...
			return new ASTNodeValue(valueHolder.getCurrentStoichiometry(nsb.getId()),this);
		}
			...
			return new ASTNodeValue(valueHolder.getValueOf(nsb.getId()), this);
    ...
    public ASTNodeValue compile(String name) {
    	...
    	if (funcArg != null) {
			...
			return new ASTNodeValue(valueHolder.getValueOf(name), this);
		}
     ...
     */
    public ASTNodeValue geq(ASTNode nodeleft, ASTNode noderight)throws SBMLException {
