...
/**
 * This is the definition of the model calibration problem for a given SBML
 * {@link Model} and experimental data in form of a {@link File} of
...
InterfaceAdditionalPopulationInformer, InterfaceHasInitRange {
  ...
  public void calculateStatisticsForGeneration() {
    ESIndividualDoubleData ind = (ESIndividualDoubleData) optimizer.getPopulation().getBestIndividual();
    bestSolutionFound = ind.getDoublePosition();
    eval(ind.getDoublePosition());
  }
  ...
  public EstimationProblem(EstimationProblem problem) {
    super(problem);
    try {
      initialize(defaultValue, negationOfDistance,
        multishoot, solver, distance,
        distance.getDefaultValue(), getModel(),
        referenceData, quantityRanges);
    } catch (Exception e) {
      // can never happen.
    }
  }
  ...
  public double[] getTimePoints() {
    if ((timePoints == null) && (referenceData.length > 0)) {
      ...
      if (referenceData.length == 1) {
        timePoints = referenceData[0].getTimePoints();
      } else {
        ...
        i = 0;
        timePoints = new double[tp.size()];
        for (Double d : tp) {
          timePoints[i++] = d.doubleValue();
        }
      }
    }
    return timePoints;
  ...
  public void setQuantityRanges(QuantityRange... quantRange) {
    if (check(quantRange)) {
      quantityRanges = quantRange;
      originalValues = new double[quantityRanges.length];
